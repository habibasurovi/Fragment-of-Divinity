...
inline void drawBoss() {
  ...
  // Draw Boss
  iShowImage((int)level4Boss.x, (int)level4Boss.y, BOSS_W, BOSS_H,
             bossImgs[level4Boss.frameIndex]);

  // Draw Flames at the bottom of the boss
  // Anchor flames to the bottom edge of the boss with a small 10px overlap/gap
  // for "continuous effect"
  int flameY = (int)level4Boss.y - 10;

  // Distribute 4 small flames across the boss base (~400px width)
  iShowImage((int)level4Boss.x + 20, flameY, BOSS_FLAME_W, BOSS_FLAME_H,
             bossFlameImgs[level4Boss.flameFrameIndex]);
  iShowImage((int)level4Boss.x + 120, flameY, BOSS_FLAME_W, BOSS_FLAME_H,
             bossFlameImgs[level4Boss.flameFrameIndex]);
  iShowImage((int)level4Boss.x + 220, flameY, BOSS_FLAME_W, BOSS_FLAME_H,
             bossFlameImgs[level4Boss.flameFrameIndex]);
  iShowImage((int)level4Boss.x + 320, flameY, BOSS_FLAME_W, BOSS_FLAME_H,
             bossFlameImgs[level4Boss.flameFrameIndex]);
}
