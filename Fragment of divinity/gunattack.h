...
inline void updateFireballs() {
  ...
  for (int i = 0; i < MAX_FIREBALLS; i++) {
    if (fireballs[i].active) {
      ...
      // -- Boss Collision (Level 4 Phase 3) --
      if (boss4Obj.active && level4Phase == 3) {
        if (fX > boss4Obj.x && fX < boss4Obj.x + BOSS_WIDTH &&
            fY > boss4Obj.y && fY < boss4Obj.y + BOSS_HEIGHT) {
          boss4Obj.life -= 2;
          spawnDamagePopup(2, boss4Obj.x + BOSS_WIDTH / 2.0f,
      ...
      for (int k = 0; k < 2; k++) {
        if (sharks[k].active && !sharks[k].isDying) {
          if (fX > sharks[k].x && fX < sharks[k].x + 70 && fY > sharks[k].y &&
              fY < sharks[k].y + 70) {
  ...
  for (int i = 0; i < MAX_FIREBALLS; i++) {
    if (fireballs[i].active) {
      ...
      if (fireballs[i].type == 0) {
        if (fireballs[i].dir == 1)
          img = blueFireball[fireballs[i].frameIndex];
        else
          img = blueFireballBack[fireballs[i].frameIndex];
      } else {
        if (fireballs[i].dir == 1)
          img = redFireball[fireballs[i].frameIndex];
        else
          img = redFireballBack[fireballs[i].frameIndex];
      }

      if (img != -1) {
        iShowImage((int)fireballs[i].x - 20, (int)fireballs[i].y - 20, 60, 60,
                   img);
      }
    }
  }
